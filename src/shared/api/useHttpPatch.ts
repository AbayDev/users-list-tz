...
/**
 * Хук для выполнения PATCH-запросов к серверу с обработкой состояния загрузки и ошибок.
 ...
 */
export function useHttpPatch<Res = unknown, Req = unknown>() {
   ...
   */
  const patch = async (url: string, body: Req, config?: AxiosRequestConfig) => {
    loading.value = true
    error.value = null

    try {
      const response = await http.patch<Res>(url, body, config)
